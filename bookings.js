...
const sumtotalElem = document.querySelector('#sumtotal');
...
const totalCalc = function(){
  ...
  //declare all initial variable
   ...
   //loop: for calculating 'sumtotal' value from every single product
   for (let i = 0; i < quantityElem.length; i++){
    sumtotal += Number(quantityElem[i].textContent) * Number(priceElem[i].textContent);
   }

   //show the 'sumtotal' variable value on 'sumtotalElem element
   sumtotalElem.textContent = sumtotal.toFixed(2);

   //calculating the 'totalTax'
   totalTax = sumtotal * tax;

   //show the 'totalTax' on 'taxElem' element
   taxElem.textContent = totalTax.toFixed(2);

   //calculating the 'total'
   total = sumtotal + totalTax;
